...
def lemmatize_pos(x):   #TOFIX: might be slow on large data sets?
    """
...
def prepare_text(text):
    ...
    creates mc-midprep-* file that is in generic data format for rest of pipeline
    """
...
def build_dict_replace_words(row, mdict, custom_stopwords, index):
    ...
    replaces words in corpus with stems and
    writes stemmed corpus to file
    """
...
def count_words(row, mdict, name):
    ...
    # read speech acts from file and create sa vector
...
def pipeline_prepare_data(text):
    """
